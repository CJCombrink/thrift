...
void t_cpp_generator::generate_enum(t_enum* tenum) {
  ...
  */
  ...
  f_types_ << indent() << "extern const std::map<int, const char*> " << map_type_name << ";" << endl
           << endl;
...
void t_cpp_generator::generate_enum_ostream_operator(std::ostream& out,
  ...
  out << indent() <<  "const auto it = " << map_name << ".find(val);" << endl;
  out << indent() << "if (it != " << map_name << ".end()) {" << endl;
  indent_up();
...
void t_cpp_generator::generate_struct_reader(ostream& out, t_struct* tstruct, bool pointers, e_cpp17 is_cpp17) {
  ...
  if (gen_templates_) {
    ...
    if (is_cpp17 == e_cpp17::NO) {
      out << indent() << "uint32_t " << tstruct->get_name() << "::read(Protocol_* iprot) {" << endl;
    } else {
...
void t_cpp_generator::generate_deserialize_set_element(ostream& out,
  ...
  indent(out) << declare_field(&felem, false, false, false, false, is_cpp17 == e_cpp17::YES) << endl;
...
void t_cpp_generator::generate_deserialize_list_element(ostream& out,
  ...
  if (use_push) {
    ...
    indent(out) << declare_field(&felem, false, false, false, true, is_cpp17 == e_cpp17::YES) << endl;
    generate_deserialize_field(out, &felem, std::string{}, std::string{}, is_cpp17);
